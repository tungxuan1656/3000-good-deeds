...
Hệ thống hỗ trợ **4 loại mục tiêu**, bỏ qua mục tiêu hàng ngày để tránh áp lực không cần thiết.
...
| `milestone` | Mục tiêu theo mốc tổng (VD: 100, 3000 việc thiện) |
...
| **G05** | Người dùng | Đặt mục tiêu milestone (VD: 3000)  | Có động lực dài hạn                           |
...
| end_date     | INTEGER | Ngày kết thúc chu kỳ (milestone: khi hoàn thành) |
...
Mục tiêu milestone:
- `start_date` = thời điểm enable
- `end_date` = NULL (cho đến khi hoàn thành)
...
2. Kiểm tra mục tiêu có đang bật (`is_enabled = true`)
3. Phân biệt rõ 2 trường hợp:

**Trường hợp A: Deed thuộc chu kỳ HIỆN TẠI**
- Tạo `goal_history` nếu chưa có (qua `ensureGoalHistory`)
- Recompute `actual_count`
- Cập nhật `completed` nếu đạt target

**Trường hợp B: Deed thuộc chu kỳ QUÁ KHỨ**
- **KHÔNG tạo mới** `goal_history`
- **CHỈ update** nếu `goal_history` đã tồn tại cho chu kỳ đó
- Nếu không tồn tại → deed chỉ là ghi nhận cá nhân, không ảnh hưởng mục tiêu
...
- user có hành động (thêm/xóa deed)
- hoặc user xem lịch sử / tiến độ

Mọi logic tạo history đều đi qua **một hàm duy nhất** để tránh trùng lặp.

---

### 5.5. Với mục tiêu Milestone

- `period_time`: cố định (VD: `milestone_1`)
- `completed = true` khi `actual_count >= target_count`
- `end_date` = thời điểm hoàn thành
- Sau khi hoàn thành:
  - mục tiêu được giữ lại như lịch sử
  - user có thể đặt milestone mới (VD: 3000 → 5000)
...
- `goal_history` hiện tại vẫn được giữ
...
- Milestone hiển thị như một hành trình liên tục
...
2. Chọn loại: Tuần / Tháng / Năm / Milestone
