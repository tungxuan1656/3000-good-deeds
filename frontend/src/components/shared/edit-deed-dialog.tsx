...
export const EditDeedDialog = React.forwardRef<EditDeedDialogHandle>(
  (_props, ref) => {
    ...
    React.useImperativeHandle(
      ...
      () => ({
        open: (d: DeedDTO) => {
          ...
          setMoodTags(
            d.labels ? d.labels.split(',').map((label) => label.trim()) : [],
          )
    ...
    const formattedDate = React.useMemo(() => {
      const value = format(selectedDate, t('dates.formats.fullDate'), {
        locale: vi,
      })
        ...
        <DialogContent>
          ...
          <div className='flex flex-col gap-4 py-4'>
            ...
            <div className='flex flex-col gap-2'>
              ...
              <p className='text-muted-foreground text-sm'>
                {t('deeds.edit.fields.moodTagsHint')}
              </p>
            </div>
          </div>
          ...
          <DialogFooter>
            ...
            <Button
              disabled={updateDeed.isPending}
              onClick={() => void handleSubmit()}>
              {updateDeed.isPending
                ? t('common.actions.saving')
                : t('deeds.edit.saveChanges')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
